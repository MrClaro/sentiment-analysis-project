...
public class SentimentController {
  ...
  public ResponseEntity<SentimentResponse> analisar(
    ...
    log.debug("Recebida requisição de análise com {} caracteres",
  ...
  @Operation(summary = "Analisa sentimento via query", description = "Endpoint simplificado para análise rápida")
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "Análise realizada", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = SentimentResponse.class))),
      @ApiResponse(responseCode = "400", description = "Texto inválido")
  })
  @GetMapping(produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<SentimentResponse> analisarPorQuery(
      @Parameter(description = "Texto para análise", required = true, example = "Este produto é excelente!") @RequestParam @NotBlank(message = "O texto não pode estar vazio") String text) {

    log.debug("Análise via query com {} caracteres", text.length());

    SentimentRequest request = SentimentRequest.builder()
        .text(text)
        .build();

    return ResponseEntity.ok(sentimentService.analisarSentimento(request));
  }
